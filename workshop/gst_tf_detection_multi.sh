...
gst-launch-1.0 videomixer name=mixer ! videoconvert ! gtksink \
    ...
    filesrc location=$FILENAME_3 ! decodebin ! videoconvert  ! video/x-raw,format=RGB ! gst_tf_detection config=$TF_MODEL_CONFIG ! videoconvert ! gst_detection_overlay ! \
    videoscale ! video/x-raw,width=$W,height=$H ! videoconvert ! videobox top=-$H left=0 ! mixer. \
    filesrc location=$FILENAME_4 ! decodebin ! videoconvert  ! video/x-raw,format=RGB ! gst_tf_detection config=$TF_MODEL_CONFIG ! videoconvert ! gst_detection_overlay ! \
